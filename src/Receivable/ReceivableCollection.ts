...
export class ReceivableCollection<Type> {
	...
	constructor(
		...
		this._items = items.sort((a, b) =>
			a.dateTime.isEqualTo(b.dateTime) ? 0 : a.dateTime.isEarlierThan(b.dateTime) ? -1 : 1,
		)
	}

	public contains(item: Receivable<Type>): boolean {
		const alreadyExistingIndex = this._items.findIndex((v) => v.id.equals(item.id))
	...
	public allocatePayment(payment: Payment): AllocatePaymentOutput<Type> {
		...
		const receivables: Items<Type> = this._items.map((receivable: Receivable<Type>) => {
		...
		return new AggregateCommandOutput(new ReceivableCollection(this.customerAccountId, receivables), events)
	}
}
